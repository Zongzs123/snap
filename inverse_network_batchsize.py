...
for train_index, val_index in kfold.split(inputs_train):
    ...
    inverse_model_size16 = inverse_network()
    inverse_model_size32 = inverse_network()
    inverse_model_size64 = inverse_network()
    inverse_model_size128 = inverse_network()
    inverse_model_size256 = inverse_network()

    # Train each model using the training data
    results_size16 = inverse_model_size16.fit(train_inputs, train_targets,
                                              batch_size=16,
                                              epochs=200,
                                              ...
                                              shuffle=True)
    results_size32 = inverse_model_size32.fit(train_inputs, train_targets,
                                              batch_size=32,
                                              epochs=200,
                                              callbacks=[lr_scheduler],
                                              verbose=2,
                                              validation_data=(val_inputs, val_targets),
                                              shuffle=True)
    results_size64 = inverse_model_size64.fit(train_inputs, train_targets,
                                              batch_size=64,
                                              epochs=200,
                                              callbacks=[lr_scheduler],
                                              verbose=2,
                                              validation_data=(val_inputs, val_targets),
                                              shuffle=True)
    results_size128 = inverse_model_size128.fit(train_inputs, train_targets,
                                                batch_size=128,
                                                epochs=200,
                                                callbacks=[lr_scheduler],
                                                verbose=2,
                                                validation_data=(val_inputs, val_targets),
                                                shuffle=True)
    results_size256 = inverse_model_size256.fit(train_inputs, train_targets,
                                                batch_size=256,
                                                epochs=200,
                                                callbacks=[lr_scheduler],
                                                verbose=2,
...
# convert loss result to dataframe
...
# Save loss result to a CSV file
base_dir = "./20231201/"
